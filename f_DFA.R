...
SSP = function(file, scale = 2^(1/8), box_size = 4, m = 1, parallel=TRUE){
  ...
  if(parallel==FALSE){
      ...
      dfa_hat = DFAp(as.vector(file), scale = scale, box_size = box_size, m = m)
  }
...
DFAp = function (file, scale = 2^(1/8), box_size = 4, m = 1){
  if (class(file) == "data.frame") {
    file <- file[, 1]
  }
  N <- length(file)
  ...
  if(numcores>1) numcores = numcores - 1 
  ...
  y_k = cumsum(file) - mean(file)
  aux_list <- pblapply(seq_along(ninbox2), function(j) {
    aux_j <- numeric(box_size[j] * ninbox2[j])
